...
const initiatePaymentForAppointment = async (appointmentId: string, user: IAuthUser) => {
     ...
     if (!appointment) {
          throw new ApiError(StatusCode.BAD_REQUEST, "Appointment not found or unauthorized");
     }

     if (appointment.paymentStatus !== PaymentStatus.UNPAID) {
          throw new ApiError(StatusCode.BAD_REQUEST, "Payment already completed for this appointment");
     }

     if (appointment.status === AppointmentStatus.CANCELED) {
          throw new ApiError(StatusCode.BAD_REQUEST, "Cannot pay for cancelled appointment");
     }
