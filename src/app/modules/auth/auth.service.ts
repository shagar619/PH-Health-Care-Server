...
const resetPassword = async (token: string | null, payload: { email?: string, password: string }, user?: { email: string }) => {
     ...
     if (token) {
          ...
          if (!decodedToken) {
               throw new ApiError(StatusCode.FORBIDDEN, "Invalid or expired reset token!")
          }

          // Verify email from token matches the email in payload
          if (payload.email && decodedToken.email !== payload.email) {
               throw new ApiError(StatusCode.FORBIDDEN, "Email mismatch! Invalid reset request.")
          }
     ...
     if (!authenticatedUser.needPasswordChange) {
          throw new ApiError(StatusCode.BAD_REQUEST, "You don't need to reset your password. Use change password instead.")
     }

     userEmail = user.email;
     } else {
          throw new ApiError(StatusCode.BAD_REQUEST, "Invalid request. Either provide a valid token or be authenticated.")
     }
